...
<script>
	...
	var TableInit = function () {
	    ...
	    oTableInit.Init = function () {
	        $('#finances').bootstrapTable({
	            url: '/manager/finance/ajaxList.do',  //请求后台的URL（*）
	            method: 'post',                     //请求方式（*）
	            ...
	            columns: [{
	                ...
	                align : 'center',
	                formatter : function (value, row, index){
	                	var result = '<button class="btn btn-default btn-xs edit" type="button" onclick="record(\''+row.id+'\');"><span class="glyphicon glyphicon-plus"></span>记录</button>'
	...
	function search(){
		$('#finances').bootstrapTable('refresh');
	}
	
	function del(){
		var arr = new Array();
		$("#finances tr.selected").each(function(){
			arr.push($(this).attr("data-uniqueid"));
		});
		var ids = arr.join(",");
		if (ids != "" && confirm("确定要进行删除操作吗？")) {
			$.ajax({
				url:"/manager/finance/remove.do",
				data:{
					id:ids
				},
				success:function(){
	...
	function record(id){
	    location.href="/manager/financerecord/list.do?financeId="+id;
	}
</script>
<div id="include">
	<#include "/manager/finance/add.ftl">
	<#include "/manager/finance/edit.ftl">  
</div>
</body>
</html>