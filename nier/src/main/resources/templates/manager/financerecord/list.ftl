            	...
            	<div class="panel-body">
                	<form id="formSearch" class="form-horizontal">
                    	<div class="form-group" style="margin-top:15px">
                            ...
                            <label class="control-label col-sm-1" for="search_sdate">sdate</label>
                            <div class="col-sm-3">
                                <input type="text" class="form-control datepicker" id="search_sdate">
                            </div>
                            
                            <label class="control-label col-sm-1" for="search_edate">edate</label>
                            <div class="col-sm-3">
                                <input type="text" class="form-control datepicker" id="search_edate">
                            </div>
...
<script>
	...
	var TableInit = function () {
	    ...
	    oTableInit.Init = function () {
	        $('#financeRecords').bootstrapTable({
	            url: '/manager/financerecord/ajaxList.do',  //请求后台的URL（*）
	            method: 'post',                     //请求方式（*）
	...
	function search(){
		$('#financeRecords').bootstrapTable('refresh');
	}
	
	function del(){
		var arr = new Array();
		$("#financeRecords tr.selected").each(function(){
			arr.push($(this).attr("data-uniqueid"));
		});
		var ids = arr.join(",");
		if (ids != "" && confirm("确定要进行删除操作吗？")) {
			$.ajax({
				url:"/manager/financerecord/remove.do",
				data:{
					id:ids
				},
				success:function(){
...
<div id="include">
	<#include "/manager/financerecord/add.ftl">
	<#include "/manager/financerecord/edit.ftl">  
</div>
</body>
</html>